...
async function createSwiperMovie() {
  if (!movieSwiperImage.value.trim()) {
    alert("Some inputs are empty!");
    return;
  }
...
function addDeleteEvent() {
  ...
  deleteMovieBtns.forEach((btn) =>
    btn.addEventListener("click", deleteSwiperMovie)
  );
}
...
function addEditEvent() {
  ...
  editMovieBtns.forEach((btn) =>
    btn.addEventListener("click", addMovieDataToForm)
  );
}
...
async function addDateToDropdownMenu() {
  ...
  dateList.innerHTML =
    '<li><a class="dropdown-item dropdown-item-date text-black" href="#">all</a></li>';
  dates.forEach((date) => {
    dateList.innerHTML += `
